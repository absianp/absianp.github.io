...
class PerformanceTracker:
    ...
    def get_post_details(self) -> list:
        ...
        for f in files:
            try:
                ...
                category = category_match.group(1).strip() if category_match else "일반"
                posts.append({
                    "slug": slug,
                    "title": title,
                    "category": category,
    ...
    def get_traffic_sources_status(self) -> Dict[str, Any]:
        ...
        repo = os.getenv("GITHUB_REPO") or self.config.get("github", {}).get("repo", "absianp/absianp.github.io")
