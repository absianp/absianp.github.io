...
class PerformanceTracker:
    """
    블로그 포스팅 개수, 예상 트래픽, 애드센스 실제/예상 수익, 라즈베리파이 하드웨어 헬스 상태를 수집하는 에이전트
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.adsense_api = GoogleAdSenseAPI(config)
    ...
    def get_adsense_statistics(self) -> Dict[str, Any]:
        # 1. 실제 구글 애드센스 API 연동 시도 (토큰/키 존재 시 자동 활성화)
        live_stats = self.adsense_api.fetch_live_statistics()
        if live_stats:
            return live_stats

        # 2. 미연동 또는 심사 대기 중일 때는 추정 시뮬레이션 모델 제공
        post_counts = self.count_posts()
        total_posts = post_counts["total"]
        est_pv = total_posts * 45 + 120
