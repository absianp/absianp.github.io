...
💡 <i>매일 정해진 스케줄(아침 07:00 작성, 08:00/19:00 브리핑)로 무인 운영됩니다.</i>"""
    ...
    def send_adsense_daily_report(self, revenue_data: Dict[str, Any]) -> bool:
        now_str = datetime.now().strftime("%Y-%m-%d")
        is_real = revenue_data.get("is_real_data", False)
        ...
        if is_real:
            header_badge = "💰 <b>[구글 애드센스 실시간 실적 리포트]</b>"
            source_badge = "🟢 <b>데이터 출처</b>: 구글 공식 AdSense API 실시간 연동"
            footer_text = "✨ <i>구글 애드센스 실시간 정산 데이터가 정상 집계되고 있습니다.</i>"
        else:
            header_badge = "📊 <b>[구글 애드센스 일일 리포트 (추정 모델)]</b>"
            source_badge = "ℹ️ <b>상태</b>: 심사 대기 중 (사이트 규모 기반 추정치)"
            footer_text = "💡 <i>애드센스 승인 완료 시 실제 구글 공식 API 실시간 연동으로 자동 전환됩니다.</i>"

        msg = f"""{header_badge} ({now_str})
━━━━━━━━━━━━━━━━━━━━
💵 <b>오늘 {'실제' if is_real else '예상'} 수익</b>: <b>${est_earnings:.2f} USD</b> (약 {est_krw:,}원)
