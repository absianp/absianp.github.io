...
from main_pipeline import load_config, publish_queued_draft, reconcile_queued_draft, KeywordHarvester, ContentWriter, PolicyInspector, GitHubPublisher, GoogleIndexing
import asyncio
...
SITE_URL = config.get("site", {}).get("url", "https://absianp.github.io").rstrip("/")
...
def publish_kpop_draft(draft_id, expected_review_token=None):
    try:
        result = call_kpop_worker("publish", {"draft_id": draft_id, "expected_review_token": expected_review_token})
...
async def handle_queue_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for d in pending[:4]:
        ...
        keyboard.append([
            InlineKeyboardButton(f"✅ 승인 #{item_idx}", callback_data=f"approve:{draft_id}:{article_review_token(d.get('article', {}))}"),
...
async def handle_approve_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_kpop:
        success, res = await asyncio.to_thread(publish_kpop_draft, draft["draft_id"])
    else:
        success, res = await asyncio.to_thread(publish_queued_draft, config, draft["draft_id"], human_approved=True)

    if success:
        await status_msg.edit_text(
            f"🎉 <b>[{target_blog} 공개 글·이미지 배포 확인 완료]</b>\n━━━━━━━━━━━━━━━━━━━━\n"
...
async def handle_reconcile_command(update, context):
    ...
    queue = DraftApprovalQueue()
    if not queue.get_draft(draft_id) and get_kpop_queue() and get_kpop_queue().get_draft(draft_id):
        result = await asyncio.to_thread(call_kpop_worker, "reconcile", {"draft_id": draft_id})
        success, message = result["success"], result["result"]
    else:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve:"):
        ...
        if not draft:
            peer = get_kpop_queue()
            draft = peer.get_draft(target_id) if peer else None
            is_peer = bool(draft)
        ...
        if is_peer:
            success, res = await asyncio.to_thread(publish_kpop_draft, target_id, expected_review_token=token)
        else:
